    ...
    internal static class TypeBind
    {
        ...
        private static bool CheckSingleConstraint(CSemanticChecker checker, ErrorHandling errHandling, Symbol symErr, TypeParameterType var, CType arg, TypeArray typeArgsCls, TypeArray typeArgsMeth, CheckConstraintsFlags flags)
        {
            ...
            if (var.HasRefConstraint() && !arg.IsRefType())
            {
                if (fReportErrors)
                {
                    throw errHandling.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_RefConstraintNotSatisfied, symErr, new ErrArgNoRef(var), arg);
                }
            ...
            if (var.HasValConstraint())
            {
                ...
                if (!arg.IsValType() || arg is NullableType)
                {
                    if (fReportErrors)
                    {
                        throw errHandling.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ValConstraintNotSatisfied, symErr, new ErrArgNoRef(var), arg);
                    }
            ...
            for (int j = itypeMin; j < bnds.Count; j++)
            {
                ...
                if (!SatisfiesBound(checker, arg, typeBnd))
                {
                    if (fReportErrors)
                    {
                        ...
                        // when using it.
                        Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode error;
                        if (arg.IsRefType())
                        {
                            // A reference type can only satisfy bounds to types 
                            // to which they have an implicit reference conversion
                            error = Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_GenericConstraintNotSatisfiedRefType;
                        }
                        else if (arg is NullableType nubArg && checker.SymbolLoader.HasBaseConversion(nubArg.GetUnderlyingType(), typeBnd))    // This is inlining FBoxingConv
                        {
                            ...
                            if (typeBnd.isPredefType(PredefinedType.PT_ENUM) || nubArg.GetUnderlyingType() == typeBnd)
                            {
                                ...
                                // parameter
                                error = Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_GenericConstraintNotSatisfiedNullableEnum;
                            }
                            else
                            {
                                ...
                                error = Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_GenericConstraintNotSatisfiedNullableInterface;
                            }
                        }
                        else
                        {
                            ...
                            error = Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_GenericConstraintNotSatisfiedValType;
                        }
            ...
            if (fReportErrors)
            {
                throw errHandling.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NewConstraintNotSatisfied, symErr, new ErrArgNoRef(var), arg);
            }
