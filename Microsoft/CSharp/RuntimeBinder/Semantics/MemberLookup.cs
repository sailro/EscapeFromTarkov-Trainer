    ...
    internal sealed class MemberLookup
    {
        ...
        private RuntimeBinderException ReportBogus(SymWithType swt)
        {
            ...
            return meth1 == null | meth2 == null
                ? GetErrorContext().Error(
                    Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BindToBogusProp1, swt.Sym.name, new SymWithType(meth1 ?? meth2, swt.GetType()),
                    new ErrArgRefOnly(swt.Sym))
                : GetErrorContext().Error(
                    Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BindToBogusProp2, swt.Sym.name, new SymWithType(meth1, swt.GetType()),
        ...
        ******************************************************************************/
        public Exception ReportErrors()
        {
            ...
            {
                // Ambiguous lookup.
                return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AmbigMember, _swtFirst, _swtAmbig);
            }

            if (_swtInaccess)
            {
                return !_swtInaccess.Sym.isUserCallable() && ((_flags & MemLookFlags.UserCallable) != 0)
                    ? GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_CantCallSpecialMethod, _swtInaccess)
            ...
            {
                return _arity > 0
                    ? GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadCtorArgCount, _typeSrc.getAggregate(), _arity)
                    : GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NoConstructors, _typeSrc.getAggregate());
            }

            if ((_flags & MemLookFlags.Operator) != 0)
            {
                return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NoSuchMember, _typeSrc, _name);
            }

            if ((_flags & MemLookFlags.Indexer) != 0)
            {
                return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadIndexLHS, _typeSrc);
            }

            if (_swtBad)
            {
                return GetErrorContext().Error((_flags & MemLookFlags.MustBeInvocable) != 0 ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NonInvocableMemberCalled : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_CantCallSpecialMethod, _swtBad);
            }
            ...
            if (_swtBadArity)
            {
                ...
                switch (_swtBadArity.Sym.getKind())
                {
                    case SYMKIND.SK_MethodSymbol:
                        ...
                        return GetErrorContext().Error(cvar > 0 ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadArity : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_HasNoTypeVars, _swtBadArity, new ErrArgSymKind(_swtBadArity.Sym), cvar);
                    ...
                    default:
                        Debug.Assert(_arity != 0);
                        return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_TypeArgsNotAllowed, _swtBadArity, new ErrArgSymKind(_swtBadArity.Sym));
                }
            }
