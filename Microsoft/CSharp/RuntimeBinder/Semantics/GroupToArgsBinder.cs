    ...
    internal sealed partial class ExpressionBinder
    {
        internal sealed class GroupToArgsBinder
        {
                    ...
                    else
                    {
                        ...
                        if (null == pmethBest)
                        {
                            ...
                            if (bReportErrors)
                            {
                                ...
                                {
                                    throw GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AmbigCall, pAmbig1.mpwi, pAmbig2.mpwi);
                                }
                                else
                                {
                                    // The two signatures are identical so don't use the type args in the error message.
                                    throw GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AmbigCall, pAmbig1.mpwi.MethProp(), pAmbig2.mpwi.MethProp());
                                }
                            }
                        }
                    }
            ...
            private Exception ReportErrorsOnFailure()
            {
                ...
                if (_pDuplicateSpecifiedName != null)
                {
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_DuplicateNamedArgument, _pDuplicateSpecifiedName);
                }
                ...
                if (_mpwiBogus)
                {
                    ...
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BindToBogus, _mpwiBogus);
                }
                ...
                if (_results.GetUninferableResult() || _mpwiCantInferInstArg)
                {
                    ...
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_CantInferMethTypeArgs, mwtCantInfer);
                }

                if (_mwtBadArity)
                {
                    int cvar = _mwtBadArity.Meth().typeVars.Count;
                    return GetErrorContext().Error(cvar > 0 ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadArity : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_HasNoTypeVars, _mwtBadArity, new ErrArgSymKind(_mwtBadArity.Meth()), _pArguments.carg);
                }
                ...
                if (_pInvalidSpecifiedName != null)
                {
                    ...
                    return _pGroup.OptionalObject != null && _pGroup.OptionalObject.Type is AggregateType agg
                        ...
                        ? GetErrorContext().Error(
                            Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadNamedArgumentForDelegateInvoke, agg.GetOwningAggregate().name,
                            _pInvalidSpecifiedName)
                        : GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadNamedArgument, _pGroup.Name, _pInvalidSpecifiedName);
                }

                if (_pNameUsedInPositionalArgument != null)
                {
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NamedArgumentUsedInPositional, _pNameUsedInPositionalArgument);
                }

                if (_pDelegate != null)
                {
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_MethDelegateMismatch, nameErr, _pDelegate);
                }
                ...
                {
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BindToBogus, nameErr);
                }

                if (bUseDelegateErrors)
                {
                    Debug.Assert(0 == (_pGroup.Flags & EXPRFLAG.EXF_CTOR));
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadDelArgCount, nameErr, _pArguments.carg);
                }

                if (0 != (_pGroup.Flags & EXPRFLAG.EXF_CTOR))
                {
                    Debug.Assert(!(_pGroup.ParentType is TypeParameterType));
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadCtorArgCount, _pGroup.ParentType, _pArguments.carg);
                }

                return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadArgCount, nameErr, _pArguments.carg);
            }
            ...
            private RuntimeBinderException ReportErrorsForBestMatching(bool bUseDelegateErrors, Name nameErr)
            {
                ...
                if (_pDelegate != null)
                {
                    return GetErrorContext().Error(
                        Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_MethDelegateMismatch, nameErr, _pDelegate, _results.GetBestResult());
                }

                if (bUseDelegateErrors)
                {
                    // Point to the Delegate, not the Invoke method
                    return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadDelArgTypes, _results.GetBestResult().GetType());
                }

                return GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadArgTypes, _results.GetBestResult());
            }
        }
    }
}
