    ...
    internal sealed partial class ExpressionBinder
    {
        ...
        internal Expr BindToProperty(Expr pObject, PropWithType pwt, BindingFlag bindFlags, Expr args, AggregateType pOtherType, ExprMemberGroup pMemGroup)
        {
            ...
            if ((bindFlags & BindingFlag.BIND_RVALUEREQUIRED) != 0)
            {
                if (!mwtGet)
                {
                    ...
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_PropertyLacksGet, pwt);
                }
                else
                {
                    CType type = null;
                    ...
                    if (error != ACCESSERROR.ACCESSERROR_NOERROR)
                    {
                        ...
                        {
                            throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadProtectedAccess, pwt, type, ContextForMemberLookup());
                        }
                        else
                        {
                            throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_InaccessibleGetter, pwt);
                        }
                    }
                }
            }
        ...
        LAgain:
            ...
            if (pmethBest == null)
            {
                ...
                throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AmbigCall, pmethAmbig1.mpwi, pmethAmbig2.mpwi);
            }
        ...
        private bool VerifyNamedArgumentsAfterFixed(Expr args)
        {
            ...
            while (list != null)
            {
                ...
                else
                {
                    if (seenNamed)
                    {
                        throw GetErrorContext().Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NamedArgumentSpecificationBeforeFixedArgument);
                    }
                }
            }
        ...
        private ExprOperator BadOperatorTypesError(ExpressionKind ek, Expr pOperand1, Expr pOperand2, CType pTypeErr)
        {
            ...
            if (pOperand1 != null)
            {
                if (pOperand2 != null)
                {
                    ...
                    {
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadBinaryOps, strOp, pOperand1.Type, pOperand2.Type);
                    }
                }
                else if (pOperand1.Type != null && !(pOperand1.Type is ErrorType))
                {
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadUnaryOp, strOp, pOperand1.Type);
                }
            }
        ...
        private static Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode GetStandardLvalueError(CheckLvalueKind kind)
        {
            Debug.Assert(kind >= CheckLvalueKind.Assignment && kind <= CheckLvalueKind.Increment);
            switch (kind)
            {
                case CheckLvalueKind.OutParameter:
                    return Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_RefLvalueExpected;
                case CheckLvalueKind.Increment:
                    return Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_IncrementLvalueExpected;
                default:
                    return Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AssgLvalueExpected;
            }
        }
        ...
        private void CheckPropertyAccess(MethWithType mwt, PropWithType pwtSlot, CType type)
        {
            switch (SemanticChecker.CheckAccess2(mwt.Meth(), mwt.GetType(), ContextForMemberLookup(), type))
            {
                case ACCESSERROR.ACCESSERROR_NOACCESSTHRU:
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_BadProtectedAccess, pwtSlot, type, ContextForMemberLookup());
                case ACCESSERROR.ACCESSERROR_NOACCESS:
                    throw ErrorContext.Error(mwt.Meth().isSetAccessor() ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_InaccessibleSetter : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_InaccessibleGetter, pwtSlot);
            }
        }
            ...
            switch (expr.Kind)
            {
                case ExpressionKind.Property:
                    if (kind == CheckLvalueKind.OutParameter)
                    {
                        ...
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_RefProperty);
                    }
                    ...
                    if (!prop.MethWithTypeSet)
                    {
                        ...
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AssgReadonlyProp, prop.PropWithTypeSlot);
                    }
                    break;
                ...
                case ExpressionKind.MemberGroup:
                    Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode err = (kind == CheckLvalueKind.OutParameter) ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_RefReadonlyLocalCause : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AssgReadonlyLocalCause;
        ...
        private Expr AdjustMemberObject(SymWithType swt, Expr pObject, out bool pfConstrained)
        {
            ...
            if (!bIsMatchingStatic)
            {
                if (isStatic)
                {
                    ...
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ObjectProhibited, swt);
                }

                throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ObjectRequired, swt);
            }
            ...
            if (typeObj is TypeParameterType || typeObj is AggregateType)
            {
                ...
                if (objNew == null)
                {
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_WrongNestedThis, swt.GetType(), pObject.Type);
                }
        ...
        private void checkUnsafe(CType type)
        {
            ...
            {
                throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_UnsafeNeeded);
            }
        }
