    ...
    internal sealed partial class ExpressionBinder
    {
        ...
        ***************************************************************************************************/
            ...
            if (expr.IsOK && !(dest is ErrorType))
            {
                    ...
                    expr.Type.isSimpleType() && dest.isSimpleType())
                {
                        ...
                        (ftSrc == FUNDTYPE.FT_I8 && ftDest == FUNDTYPE.FT_U8))
                    {
                        ...
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ConstOutOfRange, value, dest);
                    }
                }

                if (expr.Type is NullType && dest.fundType() != FUNDTYPE.FT_REF)
                {
                    throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ValueCantBeNull, dest);
                }
                ...
                throw ErrorContext.Error(canCast(expr.Type, dest, flags) ? Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NoImplicitConvCast : Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NoImplicitConv, new ErrArg(expr.Type, ErrArgFlags.Unique), new ErrArg(dest, ErrArgFlags.Unique));
            }
        ...
        private Expr mustCastCore(Expr expr, ExprClass destExpr, CONVERTTYPE flags)
        {
            ...
            SemanticChecker.CheckForStaticClass(null, dest, Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ConvertToStaticClass);
            if (expr.IsOK)
            {
                ...
                { // don't report cascading error.
                    ...
                    if (simpleConstToSimpleDestination && expr_type == FUNDTYPE.FT_STRUCT)
                    {
                        ...
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ConstOutOfRange, ((ExprConstant)exprConst).Val.DecimalVal.ToString(CultureInfo.InvariantCulture), dest);
                    }
                    else if (simpleConstToSimpleDestination && Context.CheckedConstant)
                    {
                        ...
                        throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_ConstOutOfRangeChecked, value, dest);
                    }
        ...
        private void CantConvert(Expr expr, CType dest)
        {
            ...
            {
                throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_NoExplicitConv, new ErrArg(expr.Type, ErrArgFlags.Unique), new ErrArg(dest, ErrArgFlags.Unique));
            }
        }
        ...
        ***************************************************************************************************/
        ...
        private Expr HandleAmbiguity(Expr exprSrc, CType typeSrc, CType typeDst, List<UdConvInfo> prguci, int iuciBestSrc, int iuciBestDst)
        {
            Debug.Assert(0 <= iuciBestSrc && iuciBestSrc < prguci.Count);
            Debug.Assert(0 <= iuciBestDst && iuciBestDst < prguci.Count);
            throw ErrorContext.Error(Microsoft.CSharp.RuntimeBinder.Errors.ErrorCode.ERR_AmbigUDConv, prguci[iuciBestSrc].mwt, prguci[iuciBestDst].mwt, typeSrc, typeDst);
        }
